...
const Clients = () => {
    ...
    const employeesData = useMemo(() => {
      ...
      computedEmployees = computedEmployees.filter(obj => obj.is_active !== deleted);
  
      if (searchTerm){
        computedEmployees = computedEmployees.filter((row) =>
    ...
    return (
      <div>
        <div className="flex items-center justify-between h-16">
            ...
            <Link to='/orders/add' type="button" className="bg-gray-100 text-gray-700 text-base px-6 py-2 rounded-lg">Nowy pracownik</Link>
        </div>
